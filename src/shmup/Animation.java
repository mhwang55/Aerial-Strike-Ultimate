...
public class Animation {
    ...
    // Array of subimages in animation
    private ArrayList<BufferedImage> imgList = new ArrayList<>();
    private boolean needImgList = false;
    ...
    public Animation(BufferedImage animImage, int frameWidth, int frameHeight, int numberOfFrames, long frameTime,
                     boolean loop, double x, double y, long showDelay)
    {
        ...
        if (numberOfFrames > 1)
        {
            ...
            {
                BufferedImage img = animImage.getSubimage(frameWidth * i, 0, frameWidth, frameHeight);
                imgList.add(img);
            }
            needImgList = true;
        }
    ...
    public void Draw(Graphics2D g2d)
    {
        ...
        if(this.timeOfAnimationCreation + this.showDelay <= System.currentTimeMillis()) {
            if (needImgList)
                g2d.drawImage(imgList.get(currentFrameNumber), null, x, y);  // the actual location of the sprite
            else
                g2d.drawImage(animImage, null, x, y);  // the actual location of the sprite
        }
            //g2d.drawImage(animImage, x, y, x + frameWidth, y + frameHeight, startingXOfFrameInImage, 0, endingXOfFrameInImage, frameHeight, null);
    }

    /**
     * Draws current frame of the animation rotated
     *
     * @param g2d Graphics2D, double angle
     */
    public void Draw(Graphics2D g2d, double angle, int locX, int locY)
    {
        ...
        trans.rotate( rotationRequired, locX + x, locY + y); // the points to rotate around
